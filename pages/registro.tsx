...
export default function Registro() {
  ...
  const preselect = router.query.plan as string || 'plan_monthly'
        ...
        <div style={{ background: 'white', borderRadius: 24, padding: '48px 40px', maxWidth: 520, width: '100%', boxShadow: '0 40px 100px rgba(0,0,0,0.3)' }}>
          ...
          {step === 1 && (
            <>
              ...
              <p style={{ fontSize: 14, color: '#88a8b0', marginBottom: 28 }}>Puedes cambiar o cancelar cuando quieras.</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 28 }}>
                {PLANS.map(p => (
                  <div key={p.id} onClick={() => setPlan(p.id)}
                    ...
                    {p.badge && <div style={{ position: 'absolute', top: -10, right: 16, background: '#f59e0b', color: 'white', borderRadius: 8, padding: '2px 10px', fontSize: 11, fontWeight: 800 }}>{p.badge}</div>}
          ...
          {step === 2 && (
            <form onSubmit={handleSubmit}>
              ...
              <p style={{ fontSize: 14, color: '#88a8b0', marginBottom: 28 }}>Plan <strong style={{ color: '#1a6478' }}>{PLANS.find(p=>p.id===plan)?.name}</strong> · {PLANS.find(p=>p.id===plan)?.price}€{PLANS.find(p=>p.id===plan)?.period}</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 24 }}>
                <div>
                  ...
                  <input style={inputStyle} value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} placeholder="Ej: Restaurante El Rincón" required
              ...
              <p style={{ fontSize: 12, color: '#88a8b0', textAlign: 'center', marginTop: 14 }}>Pago seguro con Redsys · SSL cifrado</p>
