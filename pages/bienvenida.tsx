    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#fdf0e8,#e8f7f6)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: 'var(--font-gabarito), system-ui, sans-serif' }}>
        <div style={{ background: 'white', borderRadius: 24, padding: '56px 48px', maxWidth: 480, width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(238,117,40,0.15)' }}>
          ...
          <h1 style={{ fontSize: 28, fontWeight: 800, color: '#1e1e1e', marginBottom: 12 }}>¡Pago completado!</h1>
          <p style={{ fontSize: 16, color: '#88a8b0', lineHeight: 1.7, marginBottom: 32 }}>
            {loading ? 'Cargando los datos de tu suscripción...' : cfg.message}
          </p>
          {!loading && (
            <>
              <div style={{ background: '#fdf0e8', borderRadius: 16, padding: '20px 24px', marginBottom: 28, border: '1px solid rgba(238,117,40,0.2)' }}>
                <div style={{ fontSize: 13, color: '#88a8b0', marginBottom: 6 }}>{cfg.panelLabel}</div>
                <div style={{ fontSize: 18, fontWeight: 700, color: '#c85f1b' }}>{cfg.panelUrl}</div>
              </div>
              <a href={cfg.loginUrl} style={{ display: 'block', background: 'linear-gradient(135deg,#ee7528,#c85f1b)', color: 'white', borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 700, marginBottom: 12, textDecoration: 'none' }}>
