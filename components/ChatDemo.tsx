...
export type MensajeChatDemo = { rol: 'cliente' | 'agente'; texto: string }

const CONVERSACION_DEFAULT: MensajeChatDemo[] = [
...
type ChatDemoProps = {
  nombre?: string
  conversacion?: MensajeChatDemo[]
}

export default function ChatDemo({ nombre = 'Peluquería Demo', conversacion = CONVERSACION_DEFAULT }: ChatDemoProps) {
  ...
  return (
    <div style={{ background: '#1c1f27', border: '1px solid #2e333f', borderRadius: 18, padding: 20, width: '100%', maxWidth: 360, display: 'flex', flexDirection: 'column', gap: 10, minHeight: 300 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingBottom: 10, borderBottom: '1px solid #2e333f', marginBottom: 4 }}>
        ...
        <div>
          <div style={{ fontSize: 14, color: '#ece9e2', fontWeight: 600 }}>{nombre}</div>
